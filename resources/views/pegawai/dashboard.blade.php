...
<div class="attendance-history">
    ...
    <div>
        ...
        <div class="history-item">
            ...
            <i class="fas fa-chevron-right text-info cursor-pointer"
               data-bs-toggle="modal"
            ...
            <div class="detail-info-section">
                <div class="detail-datetime-info">
                    ...
                    <div class="detail-day">
                        {{ \Carbon\Carbon::parse($p->created_at)->translatedFormat('l') }},
        ...
        <div class="modal-content rounded-2xl">
            <div class="modal-body p-0">
                <div class="text-center p-0">
                    <div class="w-20 h-20 bg-red-100 rounded-full flex items-center justify-content-center mx-auto mb-4">
        ...
        <div class="modal-content rounded-2xl">
            <div class="modal-body p-0">
                <div class="text-center p-6">
                    <div class="w-20 h-20 bg-yellow-100 rounded-full flex items-center justify-content-center mx-auto mb-4">
...
@push('styles')
<style>
    .confirmation-icon { animation: pulse 2s infinite; }
    @keyframes pulse { 0%{transform:scale(1)} 50%{transform:scale(1.1)} 100%{transform:scale(1)} }
    .confirmation-details { border-left: 4px solid #ffc107; }
    ...
    .toast.bg-success { background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important; }
    .toast.bg-danger { background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%) !important; }
    .toast.bg-warning { background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%) !important; }
    ...
    .modal-body .flex button:first-child { border-bottom-left-radius: 16px; }
    .modal-body .flex button:last-child { border-bottom-right-radius: 16px; }
</style>
@endpush
...
<script>
    ...
    function handlePulangClick() {
        if (!sudahPresensiMasuk) {
            ...
            if (warningEl && window.bootstrap?.Modal) {
                new bootstrap.Modal(warningEl).show();
            } else {
    ...
    function updateLocationInfo(position) {
        ...
        if (distance <= radius) {
            if (infoEl) infoEl.innerHTML = '<span class="badge bg-success">✔ Anda berada di dalam wilayah kerja</span>';
            if (submitBtn) submitBtn.disabled = false;
            if (addrEl) addrEl.textContent = wilayahAlamat || 'Di dalam wilayah kerja';
            isOutsideRadius = false;
        } else {
            if (infoEl) infoEl.innerHTML = '<span class="badge bg-warning">⚠ Anda berada di luar radius wilayah kerja</span>';
            if (submitBtn) submitBtn.disabled = false;
    ...
    function initializeMiniMap(position) {
        ...
        if (!mapInstance) {
            mapInstance = L.map(mapEl, { zoomControl: false, attributionControl: false }).setView([lat, lng], 17);
    ...
    function initializeMiniMapWithDefault() {
        ...
        if (!mapInstance) {
            mapInstance = L.map(mapEl, { zoomControl: false, attributionControl: false }).setView([-6.2088, 106.8456], 10);
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19 }).addTo(mapInstance);
            setTimeout(() => { try { mapInstance.invalidateSize(); } catch (e) {} }, 300);
        }
    }
