...
<script>
    ...
    // === LOKASI — pola darurat: simple, no complexity ===
    ...
    function initializeLocation() {
        // 1. Buat map langsung — tidak tunggu GPS
        ...
        if (mapEl && window.L && !mapInstance) {
            ...
            mapInstance = L.map(mapEl, { zoomControl:false, attributionControl:false, dragging:false, scrollWheelZoom:false }).setView([dLat, dLng], 15);
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {maxZoom:19}).addTo(mapInstance);
            setTimeout(function(){ if(mapInstance) try{mapInstance.invalidateSize();}catch(e){} }, 200);
            setTimeout(function(){ if(mapInstance) try{mapInstance.invalidateSize();}catch(e){} }, 600);
        }

        // 2. Start GPS watch — update terus
        if (!navigator.geolocation) {
            var addrEl = document.getElementById('location-address-mini');
            if (addrEl) addrEl.innerHTML = '<i class="fas fa-times-circle" style="color:var(--danger);"></i> Browser tidak mendukung geolokasi';
            return;
        }
        ...
        _locationWatchId = navigator.geolocation.watchPosition(function(pos) {
            ...
            if (!currentPosition) {
                var addrEl = document.getElementById('location-address-mini');
                if (addrEl) addrEl.innerHTML = '<i class="fas fa-times-circle" style="color:var(--danger);"></i> Gagal deteksi lokasi — <span onclick="retryLocation()" style="text-decoration:underline;cursor:pointer;font-weight:600;">Coba lagi</span>';
            }
